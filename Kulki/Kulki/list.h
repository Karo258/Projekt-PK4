...
struct element_listy {
    int wynik;																		//przechowywana w liœcie wartoœæ - wynik
    std::shared_ptr<element_listy> pNext;													//wskaŸnik na kolejny element listy
    element_listy(int liczba_punktow) : wynik(liczba_punktow), pNext(nullptr) {};	//konstruktor elementu listy
};
...
public:
    void dodaj_na_koniec(int liczba_punktow);	//metoda realizuj¹ca dodawanie elementów na koniec listy jednokierunkowej
	...
	void dodaj_w_odpowiednim_miejscu(int wynik);		//metoda dodaj¹ca element na w³aœciwe miejsce w liœcie
};
